...
import { Compass, MessageSquare, Palette, User } from 'lucide-react';
  ...
  const items: TabBarProps['items'] = useMemo(
    () =>
      [
        ...
        {
          icon: (active: boolean) => (
            <Icon className={active ? styles.active : undefined} icon={Palette} />
          ),
          key: SidebarTabKey.Painting,
          onClick: () => {
            router.push('/painting');
          },
          title: t('tab.painting'),
        },
        {
          icon: (active: boolean) => (
